...
function DashboardTab() {
    ...
    const handleDelete = async (item) => {
        ...
        try {
            await deleteDoc(doc(firestore, "menu", item.id));
    ...
    const handleSave = async () => {
        ...
        try {
            await setDoc(doc(firestore, "menu", newItem.id), newItem, { merge: true });
            ...
            <div className="container mx-auto my-5">
                <div className="tab container mx-auto">
                    <Tabs defaultIndex={0}>
                            ...
                            <Tab>
                                <button
                                    ...
                                    <div className="d-flex align-items-center">
                                        <MdOutlineProductionQuantityLimits /> Menu
                                    </div>
                                </button>
                            </Tab>
                            <Tab>
                        ...
                        <TabPanel>
                            <div className='mb-4'>
                                <h1 className='text-center mb-5 text-3xl font-semibold underline'>Menu Items Details</h1>
                                <div className="d-flex justify-content-end my-2">
                                    <Link to="/addMenu"
                                        type="button"
                                        className="btn btn-danger">
                                        <div className="d-flex align-items-center">
                                            Add Menu <FaCartPlus size={20} />
                                        </div>
                                    </Link>
                                </div>
                                    ...
                                    <table className="table text-sm">
                                        ...
                                        <tbody>
                                            {documents.map((item, i) => (
                                                <tr key={i}>
                                                    <td>{i + 1}</td>
                                                    <td> <Image className='w-16 rounded-circle' src={item.url} alt="img" style={{ width: 40, height: 40 }} /></td>
                                                    <td>{item.title}</td>
                                                    <td>${item.price}</td>
                                                    <td>{item.category}</td>
                                                    ...
                                                    <td>
                                                        <button style={{ border: "none", backgroundColor: "white" }} disabled={isProcessing}>
                                                            {!isProcessing
                                                                ? <AiFillDelete className='mx-1 primary-hover' size={24} style={{ cursor: 'pointer', color: 'black' }} onClick={() => handleDelete(item)} />
                                                                : <div className='spinner-grow spinner-grow-sm'></div>}
                                                        </button>
                                                        <button style={{ border: "none", backgroundColor: "white" }} disabled={isProcessing}>
                                                            {!isProcessing
                                                                ? <BiEdit className='primary-hover' size={24} style={{ cursor: 'pointer', color: 'black' }} data-bs-toggle='modal' data-bs-target='#exampleModalEdit' onClick={() => setItem(item)} />
                                                                : <div className='spinner-grow spinner-grow-sm'></div>}
                                                        </button>
